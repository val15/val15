...
MaFenetre::MaFenetre() : QWidget()
{
   ...
   m_btnconnexion=new QPushButton("connexion");// (D)Connecte la voie srie (bouton checkable)
    ...
    m_pb_gereCalentdier=new QPushButton("gérer calendrier");
  ...
  //  QObject::connect(m_pb_allumerEtindreLed , SIGNAL(clicked()), this,SLOT(allumerEtindreLed()));
   ...
   m_serveur->lancement();
   /*m_serveurDebit=new Serveur(7115);
   QObject::connect(m_serveur,SIGNAL(emmetreMessageRecu(QString,int,int)),this,SLOT(traiterMessagServeur(QString,int,int)));
   QObject::connect(m_serveur,SIGNAL(emmetreDeconnecionClient()),this,SLOT(deconnectionClient()));
   m_serveurDebit->lancement();*/
...
void MaFenetre::repondreAuRequete(QStringList rqt,int indiceEnvoyeur,int typeDuSeveur)
{
   ...
   if(type=="demandeActiVOuDesactiVVitesseAuto")
   {
       m_chbActiverAutoLedVitesse->setChecked(!m_chbActiverAutoLedVitesse->isChecked());//petite subtiliter "on simule le check ou le non check"
       actiVOuDesactiVVitesseAuto();
   }

   if(type=="demandeActiVOuDesactiVEspace")
       actiVOuDesactiVEspace();

   if(type=="demandeActiVOuDesactiVVitesse")
       actiVOuDesactiVVitesse();//

   if(type=="typeDebit")
       exeLectureTypeDebit(rqt);
...
{
    //qDebug() << "REP LECTURE DATES IMPORTANT DE "<<year << month;
...
void MaFenetre::exeLectureTypeDebit(QStringList lst)//A IMPLANTER
{
    m_cbTypeDebit->setCurrentText(lst.at(1));
    ...
    {
        m_estDebitInterne=true;
    }
}
void MaFenetre::exeLectureDebit(QStringList lst)//13-01
{
    m_debit=lst.at(1);
...
void MaFenetre::connection()
{
    ...
    if(!m_estConnecter)
    {
       ...
       m_arduinoCommande=new Arduino(m_portArd1,"9600");
...
void MaFenetre::actiVOuDesactiVEspace()
{
    ...
    {
        m_etatEspace="a";
       m_btEspace->setText("désactiver led espace faible");
    }
...
void MaFenetre::setRecepton(QString txtRecu)
{
   ...
   // qDebug() << txtRecu << endl;
    ...
    {
        m_etindreAlrme="d";//pour eteindre l'alarme
        m_etatClignotement="d";//pour éteidre le clignotement
       // qDebug() << txtRecu << endl;
    }
/*

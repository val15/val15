...
void MaFenetre::traiterMessagServeur(QString msg)//7-1-17
{

    //qDebug() << msg << endl;
   /* if(msg=="test")
        m_serveur->envoyer("rep test");*/
    //on reçois la requette et on le change en tableau
    QStringList rqt=msg.split("|");
    repondreAuRequete(rqt);

}
void MaFenetre::repondreAuRequete(QStringList rqt)
{
...
void MaFenetre::repLectureDesDatesImportantesDuMois(int year,int month)
{
    QStringList lstDateEvenementDuMois;
    QStringList lstRdvDuMois;
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
    ...
    rep+=lstDateEvenementDuMois.join("|");//on envois les évemements
    rep+="|rdv|";//une sorte de separateur
    rep+=lstRdvDuMois.join("|");//on envoi les rdv
    if(!lstRdvDuMois.isEmpty())
        rep+="|";
    m_serveur->envoyer(rep);
...
void MaFenetre::repLectureDesEvenementsDuJour(QString date)
{
...
void MaFenetre::repLectureDesEvenements()
{
    //QMessageBox::critical(0, QObject::tr("query Error"),"query.lastError().text()");
...
void MaFenetre::exeAjouterEnregisterEtRelecture(QStringList rqt)
{
    exeAjouterEnregister(rqt);
    repLectureDesEvenements();//pour actualiser l'affichage du client
...
void MaFenetre::exeSupprimerEtRelecture(QStringList rqt)
{
        exeSupprimer(rqt);
        repLectureDesEvenements();//pour actualiser l'affichage du client
}
void MaFenetre::exeSupprimerEvenementEtRelectureEvenementDuJour(QStringList rqt)
{
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
    db.setDatabaseName(m_cheminEtNomDb);
    if(!db.open())
    ...
    lectureDesEvenementsDuJour();
    lireEvenementDuJourEtProch1();
    repLectureDesEvenementsDuJour(rqt.at(2));
}
...
void MaFenetre::exeModifierEvenementEtRelecture(QStringList rqt)
{
    exeModifierEvenement(rqt);
    repLectureDesEvenements();//pour actualiser l'affichage du client
}
void MaFenetre::exeModifierEvenementEtRelectureEvenementDuJour(QStringList rqt)
{
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
    db.setDatabaseName(m_cheminEtNomDb);
   // QMessageBox::critical(0, QObject::tr("Database Error"),m_cheminEtNomDb);
    ...
    query.clear();
    db.close();
    repLectureDesEvenementsDuJour(rqt.at(6));
    lectureDesDatesImportantsDuMois();
    lectureDesEvenementsDuJour();
    lireEvenementDuJourEtProch1();
...
void MaFenetre::ecrirConfig()
{
    ...
    // QMessageBox::critical(0, QObject::tr("Database Error"),"");//31
     ...
     fichier.close();
     if(m_serveur->getUnCientConnecT())
         repLectureConfig();
}

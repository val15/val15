...
void Serveur::donneesRecues()
{
    ...
    {
        if (socket->bytesAvailable() < (int)sizeof(quint64)) // On n'a pas reu la taille du message en entier
             return;
  ...
  //  m_teInfo->append("message recu :" +message);
...
void Serveur::envoyer(const QString &message)
{
    ...
    out << (quint64) 0; // On crit 0 au dbut du paquet pour rserver la place pour crire la taille
    out << message; // On ajoute le message  la suite
    out.device()->seek(0); // On se replace au dbut du paquet
    out << (quint64) (paquet.size() - sizeof(quint64)); // On crase le 0 qu'on avait rserv par la longueur du message:taille total- taille d'un int
    client->write(paquet);
}
